...
class AppPresser_Ajax_Extras extends AppPresser {
	...
	public function appp_reset_password() {
		...
		file_put_contents("/tmp/debug.log", "AJAX APPP_RESET_PASSWORD called\n", FILE_APPEND);
		$nonce = $_POST['nonce'];
		$email = $_POST['email'];

		if ( !wp_verify_nonce( $nonce, 'new_password' ) ) {
			file_put_contents("/tmp/debug.log", "NOT GENERATED, NONCE ISSUE?\n", FILE_APPEND);
			return;
		}
		...
		if( $user ) {
			...
			file_put_contents("/tmp/debug.log", "GENERATED!!!\n", FILE_APPEND);
			...
			file_put_contents("/tmp/debug.log", "NOT GENERATED, EMAIL ISSUE?\n", FILE_APPEND);
			$return = array(
	...
	public function appp_validate_password_code() {
		...
		file_put_contents("/tmp/debug.log", "AJAX function called\n", FILE_APPEND);
		...
		if ( !wp_verify_nonce( $nonce, 'new_password' ) ){
			file_put_contents("/tmp/debug.log", "Raw Nonce: " . print_r($nonce, true) . "\n", FILE_APPEND);
			$result = wp_verify_nonce($nonce, 'new_password');
			file_put_contents("/tmp/debug.log", "Nonce Check Result: " . print_r(wp_verify_nonce($nonce, 'new_password'), true) . "\n", FILE_APPEND);
    		file_put_contents("/tmp/debug.log", "no nonce, ha ha\n", FILE_APPEND);
			return;
		} 
    	...
    	wp_send_json_success(['message' => 'Function executed!']);
	}
